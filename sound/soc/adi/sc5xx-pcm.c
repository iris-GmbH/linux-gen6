...
static struct snd_pcm_ops sc5xx_pcm_ops = {
	.open		= sc5xx_pcm_open,
...
{
	return snd_soc_register_platform(&pdev->dev, &sc5xx_soc_platform);
}

static int sc5xx_soc_platform_remove(struct platform_device *pdev)
{
...
static struct platform_driver sc5xx_pcm_driver = {
	...
	.remove = sc5xx_soc_platform_remove,
};
