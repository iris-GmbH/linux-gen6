...
#define REG_PINT_MSK_SET  REG_PINT0_MSK_SET
#define REG_PINT_MSK_CLR  REG_PINT0_MSK_CLR
...
#define REG_PINT_MSK_SET  REG_PINT4_MSK_SET
#define REG_PINT_MSK_CLR  REG_PINT4_MSK_CLR
...
static int sx127x_setlorasyncword(struct sx127x *data, unsigned syncw){
	u8 r;
	dev_info(data->chardevice, "setting lora sync word to %u\n", syncw);
...
//	writel(INTERRUPT_PORT, __io_address(REG_PINT_MSK_SET)); //enable IRQ
	return 0;
...
//	writel(INTERRUPT_PORT, __io_address(REG_PINT_MSK_CLR)); //disable IRQ
	return 0;
}
...
static int __init sx127x_init(void)
{
	...
	if(ret < 0){
		...
		printk("sx127x_init: char device registered\n");
	...
	if(!devclass){
		...
		printk("sx127x_init: class registered\n");
	...
	if(ret){
		...
		printk("sx127x_init: spi driver registered\n");
