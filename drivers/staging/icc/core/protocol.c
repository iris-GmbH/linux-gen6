	...
	int timeout, int nonblock)
{
	int ret;
	mutex_lock(&bfin_icc->sessions_table->lock);
	bfin_icc->sessions_table->b_query_finish = 0;
	mutex_unlock(&bfin_icc->sessions_table->lock);
	...
	if (bfin_icc->sessions_table->b_query_finish)
		return 0;
	if (nonblock) {
		return -EAGAIN;
	} else {
		...
		ret = wait_event_interruptible_timeout(
					bfin_icc->sessions_table->query_wait,
					bfin_icc->sessions_table->b_query_finish == 1,
					timeout);
		if (!ret) {
...
{
	struct bfin_icc *icc = (struct bfin_icc *)dev_instance;
	struct sm_icc_desc *icc_info;
	int i;
