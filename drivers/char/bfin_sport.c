...
static irqreturn_t sport_rx_handler(int irq, void *dev_id)
{
	...
	if (dev->rx_received && dev->rx_received >= dev->rx_len) {
		dev->regs->rcr1 &= ~RSPEN;
		complete(&dev->c);
	}
...
static int sport_open(struct inode *inode, struct file *filp)
{
	...
	if (dev->config.mode != NDSO_MODE) {
		ret = maybe_request_irq(dev->tx_irq, sport_tx_handler,
				IRQF_SHARED, KBUILD_MODNAME "-tx", dev);
		if (ret)
			goto fail;

		ret = maybe_request_irq(dev->rx_irq, sport_rx_handler,
				IRQF_SHARED, KBUILD_MODNAME "-rx", dev);
		if (ret)
			goto fail1;

		ret = maybe_request_irq(dev->err_irq, sport_err_handler,
				0, KBUILD_MODNAME "-err", dev);
		if (ret)
			goto fail2;
	}
		...
		loff_t *f_pos)
{
	...
	struct sport_config *cfg = &dev->config;
	int status;
	dma_addr_t dma_rx_addr, dma_tx_addr;
	...
	if (cfg->dma_enabled) {
		...
		dma_rx_addr = dma_map_single(dev->dev, sport_rx_buf,
		...
		set_dma_start_addr(dev->dma_rx_chan, dma_rx_addr);
		...
		if (cfg->mode == TDM_MODE) {
			...
			dma_tx_addr = dma_map_single(dev->dev, sport_tx_buf,
					count, DMA_TO_DEVICE);
			set_dma_start_addr(dev->dma_tx_chan, dma_tx_addr);
	...
	if (cfg->dma_enabled) {
		dma_unmap_single(dev->dev, dma_rx_addr,	count,
			DMA_FROM_DEVICE);
		dma_unmap_single(dev->dev, dma_tx_addr,	count,
			DMA_TO_DEVICE);
		copy_to_user(buf, sport_rx_buf, count);
	}
		...
		size_t count, loff_t *f_pos)
{
	...
	if (cfg->dma_enabled) {
		...
		copy_from_user(sport_tx_buf, buf, count);
...
static int sport_configure(struct sport_dev *dev, struct sport_config *config)
{
	...
	if (config->mode != NDSO_MODE &&
		(old_cfg->dma_enabled == 0) && (config->dma_enabled)) {
		int ret;
		free_irq(dev->tx_irq, dev);
		free_irq(dev->rx_irq, dev);
