...
static struct bfin_display_config *adi_get_config(struct platform_device *pdev)
{
	...
	config = (struct bfin_display_config *)pdev->dev.platform_data;
	if (!config) {
...
#ifdef CONFIG_OF
	match = of_match_device(adi_disp_match, &pdev->dev);
	if (!match) {
